...
r.prototype.parse = function (t, s, h) {
	...
	if (((O = O.trim()), !h && 1 === t.split("#").length)) {
		...
		if (d) {
			return (
				...
				(this.pathname = d[1]),
				d[2]
					? ((this.search = d[2]), (this.query = s ? b.parse(this.search.substr(1)) : this.search.substr(1)))
	...
	if (!g[j] && (x || (j && !y[j]))) {
		...
		-1 !== (C = -1 === I ? O.lastIndexOf("@") : O.lastIndexOf("@", I)) &&
			((A = O.slice(0, C)), (O = O.slice(C + 1)), (this.auth = decodeURIComponent(A)));
		I = -1;
				...
				if (R && !R.match(f)) {
					...
					if (!$.match(f)) {
						var L = k.slice(0, w),
							Z = k.slice(w + 1),
							_ = R.match(m);
						_ && (L.push(_[1]), Z.unshift(_[2])), Z.length && (O = "/" + Z.join(".") + O);
	...
	if (
		(-1 !== F
			? ((this.search = O.substr(F)),
			  (this.query = O.substr(F + 1)),
			  s && (this.query = b.parse(this.query)),
			  (O = O.slice(0, F)))
			: s && ((this.search = ""), (this.query = {})),
		O && (this.pathname = O),
		y[q] && this.hostname && !this.pathname && (this.pathname = "/"),
...
r.prototype.format = function () {
	...
	this.host
		? (r = t + this.host)
		: this.hostname &&
		  ((r = t + (-1 === this.hostname.indexOf(":") ? this.hostname : "[" + this.hostname + "]")),
	...
	return (
		s && ":" !== s.substr(-1) && (s += ":"),
		this.slashes || ((!s || y[s]) && !1 !== r)
			? ((r = "//" + (r || "")), h && "/" !== h.charAt(0) && (h = "/" + h))
			: r || (r = ""),
		e && "#" !== e.charAt(0) && (e = "#" + e),
		n && "?" !== n.charAt(0) && (n = "?" + n),
		s +
...
r.prototype.resolveObject = function (t) {
	...
	if (
		(A &&
			((h.hostname = ""),
			(h.port = null),
			h.host && ("" === x[0] ? (x[0] = h.host) : x.unshift(h.host)),
			(h.host = ""),
			t.protocol &&
				((t.hostname = null),
				(t.port = null),
		...
		if (A) {
			(h.hostname = h.host = x.shift()),
				(U = !!(h.host && h.host.indexOf("@") > 0) && h.host.split("@")) &&
					((h.auth = U.shift()), (h.host = h.hostname = U.shift()));
		}
		return (
			(h.search = t.search),
			(h.query = t.query),
			(a.isNull(h.pathname) && a.isNull(h.search)) ||
				(h.path = (h.pathname ? h.pathname : "") + (h.search ? h.search : "")),
			(h.href = h.format()),
			h
		);
	}
	...
	for (
		var C = x.slice(-1)[0],
			I = ((h.host || t.host || x.length > 1) && ("." === C || ".." === C)) || "" === C,
			w = 0,
			N = x.length;
		N >= 0;
		N--
	...
	!j || "" === x[0] || (x[0] && "/" === x[0].charAt(0)) || x.unshift(""),
		I && "/" !== x.join("/").substr(-1) && x.push("");
	var U,
		k = "" === x[0] || (x[0] && "/" === x[0].charAt(0));
	A &&
		((h.hostname = h.host = k ? "" : x.length ? x.shift() : ""),
		(U = !!(h.host && h.host.indexOf("@") > 0) && h.host.split("@")) &&
			((h.auth = U.shift()), (h.host = h.hostname = U.shift())));
	return (
		(j = j || (h.host && x.length)) && !k && x.unshift(""),
		x.length ? (h.pathname = x.join("/")) : ((h.pathname = null), (h.path = null)),
		(a.isNull(h.pathname) && a.isNull(h.search)) ||
			(h.path = (h.pathname ? h.pathname : "") + (h.search ? h.search : "")),
...
function dew() {
	...
	var posix = {
		...
		isAbsolute: function isAbsolute(path) {
			assertPath(path);
			return (
				path.length > 0 && path.charCodeAt(0) === 47
				/*/*/
			);
		},
		join: function join() {
			...
			for (; i <= length; ++i) {
				if (i === length) {
					if (toLen > length) {
						...
						if (
							from.charCodeAt(fromStart + i) === 47
							/*/*/
						) {
...
function fileURLToPath$1(path) {
	if (typeof path === "string") {
		...
		throw new TypeError("invalid argument path , must be a string or URL");
	}
	...
	for (var n = 0; n < pathname.length; n++) {
		if (pathname[n] === "%") {
			...
			if (
				...
				// 5c 5C \
				throw new TypeError("must not include encoded \\ or / characters");
			}
		}
	}
...
function pathToFileURL(filepath) {
	...
	// path.resolve strips trailing slashes so we must add them back
	var filePathLast = filepath.charCodeAt(filepath.length - 1);
	if ((filePathLast === CHAR_FORWARD_SLASH || isWindows) && resolved[resolved.length - 1] !== exports$1.sep) {
		resolved += "/";
	}
	var outURL = new URL("file://");
	if (resolved.includes("%")) {
