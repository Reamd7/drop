...
import { getOwnNonIndexProperties, ONLY_ENUMERABLE, SKIP_SYMBOLS } from "../../internal_binding/util";
...
function innerDeepEqual(val1, val2, strict, memos = memo) {
	...
	if (Array.isArray(val1)) {
		...
		if (
			isTypedArray(val1) &&
			isTypedArray(val2) &&
			TypedArrayPrototypeGetSymbolToStringTag(val1) !== TypedArrayPrototypeGetSymbolToStringTag(val2)
		) {
			return false;
		}
	...
	if (set !== null) {
		for (const { 0: key, 1: item } of map2) {
			if (typeof key === "object" && key !== null) {
				...
				!strict &&
				(!map1.has(key) || !innerDeepEqual(map1.get(key), item, false, memos)) &&
