...
function InterfaceConstructor(input, output, completer, terminal) {
	...
	Function.prototype.call.call(EventEmitter, this);
	...
	function onkeypress(s, key) {
		...
		if (key?.sequence) {
			...
			const ch = String.prototype.codePointAt(key.sequence, 0);
...
class Interface extends InterfaceConstructor {
	...
	[kAddHistory]() {
		...
		if (this.history.length === 0 || this.history[0] !== this.line) {
			if (this.removeHistoryDuplicates) {
				...
				if (dupIndex !== -1) Array.prototype.splice.call(this.history, dupIndex, 1);
			}

			Array.prototype.unshift.call(this.history, this.line);

			// Only store so many
			if (this.history.length > this.historySize) Array.prototype.pop.call(this.history);
		}
	...
	[kNormalWrite](b) {
		...
		const newPartContainsEnding = RegExp.prototype.exec.call(lineEnding, string) !== null;
		...
		if (newPartContainsEnding) {
			this[kSawReturnAt] = String.prototype.endsWith.call(string, "\r") ? Date.now() : 0;

			// Got one or more newlines; process into "line" events
			const lines = String.prototypes.plit.call(string, lineEnding);
	...
	[kInsertString](c) {
		...
		if (this.cursor < this.line.length) {
			...
			const end = String.prototype.slice.call(this.line, this.cursor, this.line.length);
			this.line = beg + c + end;
	...
	[kTabCompleter](lastKeypressWasTab, { 0: completions, 1: completeOn }) {
		...
		const prefix = commonPrefix(Array.prototype.filter.call(completions, (e) => e !== ""));
		if (String.prototype.startsWith.call(prefix, completeOn) && prefix.length > completeOn.length) {
			this[kInsertString](String.prototype.slice.call(prefix, completeOn.length));
			return;
		} else if (!String.prototype.startsWith.call(completeOn, prefix)) {
			this.line =
				String.prototype.slice.call(this.line, 0, this.cursor - completeOn.length) +
				prefix +
		...
		const completionsWidth = Array.prototype.map.call(completions, (e) => getStringWidth(e));
	...
	[kWordLeft]() {
		if (this.cursor > 0) {
			...
			const reversed = Array.prototype.join.call(Array.prototype.reverse.call(Array.from(leading)), "");
			const match = RegExp.prototype.exec.call(/^\s*(?:[^\w\s]+|\w+)?/, reversed);
	...
	[kWordRight]() {
		if (this.cursor < this.line.length) {
			...
			const match = RegExp.prototype.exec.call(/^(?:\s+|[^\w\s]+|\w+)\s*/, trailing);
			this[kMoveCursor](match[0].length);
		}
	}
	...
	[kDeleteRight]() {
		if (this.cursor < this.line.length) {
			...
			this.line =
				...
				String.prototype.slice.call(this.line, this.cursor + charSize, this.line.length);
			this[kRefreshLine]();
		}
	}
	...
	[kDeleteWordLeft]() {
		if (this.cursor > 0) {
			...
			const reversed = Array.prototype.join.call(Array.prototype.reverse.call(ArrayFrom(leading)), "");
			const match = RegExp.prototype.exec.call(/^\s*(?:[^\w\s]+|\w+)?/, reversed);
			leading = String.prototype.slice.call(leading, 0, leading.length - match[0].length);
			this.line = leading + String.prototype.slice.call(this.line, this.cursor, this.line.length);
	...
	[kDeleteWordRight]() {
		if (this.cursor < this.line.length) {
			this[kBeforeEdit](this.line, this.cursor);
			const trailing = String.prototype.slice.call(this.line, this.cursor);
			const match = RegExp.prototype.exec.call(/^(?:\s+|\W+|\w+)\s*/, trailing);
			this.line =
				String.prototype.slice.call(this.line, 0, this.cursor) + String.prototype.slice.call(trailing, match[0].length);
			this[kRefreshLine]();
		}
	}
	...
	[kDeleteLineLeft]() {
		this[kBeforeEdit](this.line, this.cursor);
		const del = String.prototype.slice.call(this.line, 0, this.cursor);
	...
	[kDeleteLineRight]() {
		this[kBeforeEdit](this.line, this.cursor);
		const del = String.prototype.slice.call(this.line, this.cursor);
	...
	[kPushToKillRing](del) {
		...
		Array.prototype.unshift.call(this[kKillRing], del);
		this[kKillRingCursor] = 0;
		while (this[kKillRing].length > kMaxLengthOfKillRing) Array.prototype.pop.call(this[kKillRing]);
	}
	...
	[kYankPop]() {
		...
		if (this[kKillRing].length > 1) {
			...
			const head = String.prototype.slice.call(this.line, 0, this.cursor - lastYank.length);
			const tail = String.prototype.slice.call(this.line, this.cursor);
	...
	[kPushToUndoStack](text, cursor) {
		if (Array.prototype.push.call(this[kUndoStack], { text, cursor }) > kMaxUndoRedoStackSize) {
			Array.prototype.shift.call(this[kUndoStack]);
		}
	}
	...
	[kUndo]() {
		...
		Array.prototype.push.call(this[kRedoStack], {
			text: this.line,
			cursor: this.cursor,
		});

		const entry = Array.prototype.pop.call(this[kUndoStack]);
	...
	[kRedo]() {
		...
		Array.prototype.push.call(this[kUndoStack], {
			text: this.line,
			cursor: this.cursor,
		});

		const entry = Array.prototype.pop.call(this[kRedoStack]);
		this.line = entry.text;
		this.cursor = entry.cursor;
	...
	[kHistoryNext]() {
		if (this.historyIndex >= 0) {
			...
			while (
				index >= 0 &&
				(!String.prototype.startsWith.call(this.history[index], search) || this.line === this.history[index])
			) {
				index--;
			}
			if (index === -1) {
	 ...
	 */
	getCursorPos() {
		const strBeforeCursor = this[kPrompt] + String.prototype.slice.call(this.line, 0, this.cursor);
	...
	[kTtyWrite](s, key) {
		...
		if ((key.name === "up" || key.name === "down") && !key.ctrl && !key.meta && !key.shift) {
			if (this[kSubstringSearch] === null) {
				this[kSubstringSearch] = String.prototype.slice.call(this.line, 0, this.cursor);
			}
		...
		if (typeof key.sequence === "string") {
			switch (String.prototype.codePointAt.call(key.sequence, 0)) {
				case 0x1f:
					this[kUndo]();
					return;
				case 0x1e:
