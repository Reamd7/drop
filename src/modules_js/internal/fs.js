...
function read(fd, buffer, offset, length, position, callback) {
	if (typeof buffer === "function") {
		...
		!Object.prototype.toString.call(buffer).endsWith("Array]") &&
		arguments.length === 3
	) {
		callback = offset;
		let option = buffer ?? {};
	...
	if (buffer.byteLength === 0) {
		throw new errors.ERR_INVALID_ARG_VALUE("buffer", buffer, "is empty and cannot be written");
	}
	validateInteger(length, "length", 0, buffer.byteLength);

	fread(fd, position, length)
		.then((data) => {
...
function write(fd, buffer, offset, length, position, callback) {
		...
		if (typeof offset === "function") {
			...
			if (length === "hex" && buffer.length % 2 === 1) {
				throw new errors.ERR_INVALID_ARG_VALUE_RANGE(
					"encoding",
					length,
					`is invalid for data of length ${buffer.length}`,
				);
			}
	...
	fwrite(fd, position, buffer.buffer.slice(offset, offset + length))
		.then((len) => {
			callback(null, len, buffer);
		})
		.catch((e) => {
...
function writeFile(file, data, options, callback) {
	...
	if (
		!(buffer instanceof Buffer) &&
		(Object.prototype.toString.call(data).endsWith("Array]") ||
...
function writev(fd, buffer, position, callback) {
	...
	fwrite(fd, position, Buffer.concat(buffer).buffer)
		.then((len) => {
			callback(null, len, buffer);
		})
		.catch((e) => {
