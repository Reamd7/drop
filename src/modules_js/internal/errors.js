...
export class ERR_HTTP_HEADERS_SENT extends Error {
	constructor(x) {
		super(`Cannot ${x} headers after they are sent to the client`);
...
export class ERR_HTTP_INVALID_HEADER_VALUE extends TypeError {
	constructor(x, y) {
		super(`Invalid value "${x}" for header "${y}"`);
...
export class ERR_INVALID_CHAR extends TypeError {
	constructor(name, field) {
		super(field ? `Invalid character in ${name}` : `Invalid character in ${name} ["${field}"]`);
...
export class ERR_STREAM_ALREADY_FINISHED extends Error {
	constructor(x) {
		super(`Cannot call ${x} after a stream was finished`);
...
export class ERR_STREAM_DESTROYED extends Error {
	constructor(x) {
		super(`Cannot call ${x} after a stream was destroyed`);
...
export class ERR_SOCKET_BAD_PORT extends RangeError {
	constructor(name, port, allowZero = true) {
		...
		super(`${name} should be ${operator} 0 and < 65536. Received ${port}.`);
...
export class ERR_INVALID_CALLBACK extends TypeError {
	constructor(object) {
		super(`Callback must be a function. Received ${JSON.stringify(object)}`);
...
export class ERR_INVALID_RETURN_VALUE extends TypeError {
	constructor(input, name, value) {
		super(`Expected ${input} to be returned from the "${name}" function but got ${buildReturnPropertyType(value)}.`);
...
export class ERR_INCOMPATIBLE_OPTION_PAIR extends TypeError {
	constructor(input, name) {
		super(`Option "${input}" cannot be used in combination with option "${name}"`);
		this.code = "ERR_INCOMPATIBLE_OPTION_PAIR";
	}
}

export const captureStackTrace = hideStackFrames(function captureStackTrace(err) {
...
export function isErrorStackTraceLimitWritable() {
	...
	return Object.prototype.hasOwnProperty(desc, "writable") ? desc.writable : desc.set !== undefined;
}
...
export class ERR_ASSERT_SNAPSHOT_NOT_SUPPORTED extends TypeError {
	constructor() {
		super("ERR_ASSERT_SNAPSHOT_NOT_SUPPORTED", `Snapshot is not supported in this context`);
	}
}

export class ERR_AMBIGUOUS_ARGUMENT extends TypeError {
	constructor(arg, msg) {
		super("ERR_AMBIGUOUS_ARGUMENT", `The ${arg} argument is ambiguous. ${msg}`);
	}
}
...
export class ERR_FS_FILE_TOO_LARGE extends RangeError {
	constructor(x) {
		super(`File size (${x}) is greater than 2 GB`);
...
export class ERR_FS_INVALID_SYMLINK_TYPE extends Error {
	constructor(x) {
		super(`Symlink type must be one of "dir", "file", or "junction". Received "${x}"`);
