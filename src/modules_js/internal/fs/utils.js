...
import { isArrayBufferView, isBigUint64Array, isDate, isUint8Array } from "../util/types";
...
import { fs as fsConstants, os as osConstants } from "../../internal_binding/constants";
...
export const nullCheck = hideStackFrames((path, propName, throwError = true) => {
...
export const validateOffsetLengthRead = hideStackFrames((offset, length, bufferLength) => {
	if (offset < 0) {
...
export const validateOffsetLengthWrite = hideStackFrames((offset, length, byteLength) => {
...
export const getValidatedPath = hideStackFrames((fileURLOrPath, propName = "path") => {
...
export const validateRmOptions = hideStackFrames((path, options, expectDir, cb) => {
...
export const validateRmOptionsSync = hideStackFrames((path, options, expectDir) => {
	options = validateRmdirOptions(options, defaultRmOptions);
	validateBoolean(options.force, "options.force");
...
export const validateRmdirOptions = hideStackFrames((options, defaults = defaultRmdirOptions) => {
	if (options === undefined) {
		return defaults;
	}
	validateObject(options, "options");
...
export const validateStringAfterArrayBufferView = hideStackFrames((buffer, name) => {
...
export const validateEncoding = hideStackFrames((value, name) => {
	let exist = ["utf8", "utf-8", "", "hex", "base64", "latin1", "usc2", "utf16le", "ascii", "base64url"].indexOf(value);
